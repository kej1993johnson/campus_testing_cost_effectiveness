...
library(zoo)
#library(rsconnect)
#rsconnect::deployApp('/Users/kj22643/Documents/Documents/Testing_UT/code/shinyapp/testing_strategy_tool/code')
...
ui <- pageWithSidebar(
    ...
    # Application title
    titlePanel("University Testing Strategy"),
    ...
    sidebarPanel(
            ...
            sliderInput("vacc_rate",
                        ...
                        min = 0,
                        max = 100,
                        value = 60, step = 5, post = "%"),
            sliderInput("init_rec",
                        "Percent immune from prior infection:",
                        min = 0,
                        max = 100,
            ...
            sliderInput("transmissibility_if_vax_inf",
                        "Relative transmissibility of individuals infected who are vaccinated:",
                        min = 0,
                        max = 100,
   ...
   output$cases_t = renderPlot( height = 800, width = 600, res = 150, {
      ...
      close_thres <- df$close_thres[1]
       gg.cases = ggplot(data = df_t) + 
         ...
         coord_cartesian(ylim = c(0, 9*N*100/100000))+
     ...
     gg.costs = ggplot(df_costs_long, aes (x = testing_policy, y = cost/1e6, fill = factor(Source))) +
       geom_bar(stat = "identity", position = "stack") + 
       xlab('Test Frequency') + 
      ...
      labs(fill = "Source of cost")+
       ...
       coord_cartesian(ylim = c(0, N/1000))+
...
output$text = renderUI({
      ...
      str2<-paste("This corresponds to", df_min$n_tests_per_week," proactive tests per week")
      str3<-paste("costing $", round(df_min$cost_per_inf_averted, 0), " per student infection averted and $",
